# AGENTS.md — Sistema de desarrollo de Fernando

> Puerta de entrada. Léelo siempre al empezar sesión.
> Los skills viven en `.Codex/skills/<nombre>/SKILL.md` y se cargan SOLO cuando el task lo requiere.
...
| Deploy | Vercel |
| Pagos | Stripe |
| IA | Codex API (`Codex-opus-4-7` / `Codex-sonnet-4-6` / `Codex-haiku-4-5`) |
...
| Integrar IA en la app | `principles` → `Codex-api` |
...
> En disco todos viven plano en `.Codex/skills/<nombre>/SKILL.md` (requisito de Codex).
...
Codex-api/          ← integración IA, streaming, caching, structured outputs
```
...
Cada skill es un `SKILL.md` con frontmatter YAML (`name`, `description`), invocable por Codex cuando el contexto lo requiera.
